...
;;; Log:
;;; 2019-11-17 Fixed some compiler errors
;;;            - defvars aria2-controller && aria2-mode-map
;;;            - added some type and group statements to defcustom vars
;;;            - changed evil-mode-quirks to do something only when
;;;              evil-emacs-state-modes is defined
...
(defvar aria2-controller nil
  "Used internally")

(defvar aria2-mode-map nil
  "Used internally")
...
(defun aria2--manage-refresh-timer ()
  ...
  (let ((buf (get-buffer aria2-list-buffer-name)))
    (cl-flet ((retimer (refresh speed)
                       ...
                       (setq aria2--current-buffer-refresh-speed speed)))
      (cond
       ((and (eq buf (window-buffer (selected-window))) ; when list has focus
             (not (eq aria2--current-buffer-refresh-speed :fast)))
        (retimer aria2-refresh-fast :fast))
       ((and (get-buffer-window buf) ; list visible but without focus
...
(defun aria2-maybe-add-evil-quirks ()
  ...
  (when aria2-add-evil-quirks
    (with-eval-after-load 'evil-states
      (if (boundp 'evil-emacs-state-modes)
          (progn
